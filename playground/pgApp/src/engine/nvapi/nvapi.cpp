...
struct NvMemoryInfoV2 {
    NvMemoryInfoV2() : version(sizeof(NvMemoryInfoV2) | (2 << 16)) {}
...
typedef NvApiStatus(NVAPICALL* PFN_NVAPI_ENUMPHYSICALGPUS)(
    NvPhysicalGpuHandle* _handle[NVAPI_MAX_PHYSICAL_GPUS], uint32_t* _gpuCount);
typedef NvApiStatus(NVAPICALL* PFN_NVAPI_GPUGETMEMORYINFO)(NvPhysicalGpuHandle* _handle,
                                                           NvMemoryInfoV2* _memoryInfo);
typedef NvApiStatus(NVAPICALL* PFN_NVAPI_GPUGETFULLNAME)(NvPhysicalGpuHandle* _physicalGpu,
                                                         char _name[64]);
...
typedef int32_t (*PFN_NVAFTERMATH_DX11_INITIALIZE)(int32_t _version, int32_t _flags,
                                                   const ID3D11Device* _device);
typedef int32_t (*PFN_NVAFTERMATH_DX11_CREATECONTEXTHANDLE)(
    const ID3D11DeviceContext* _deviceCtx, NvAftermathContextHandle** _outContextHandle);
typedef int32_t (*PFN_NVAFTERMATH_DX12_INITIALIZE)(int32_t _version, int32_t _flags,
                                                   const ID3D12Device* _device);
typedef int32_t (*PFN_NVAFTERMATH_DX12_CREATECONTEXTHANDLE)(
    const ID3D12CommandList* _commandList, NvAftermathContextHandle** _outContextHandle);
typedef int32_t (*PFN_NVAFTERMATH_RELEASECONTEXTHANDLE)(
    const NvAftermathContextHandle* _contextHandle);
typedef int32_t (*PFN_NVAFTERMATH_SETEVENTMARKER)(const NvAftermathContextHandle* _contextHandle,
                                                  const void* _markerData, uint32_t _markerSize);
typedef int32_t (*PFN_NVAFTERMATH_GETDATA)(uint32_t _numContexts,
                                           const NvAftermathContextHandle** _contextHandles,
                                           void* _outContextData);
...
NvApi::NvApi() : m_nvApiDll(NULL), m_nvGpu(NULL), m_nvAftermathDll(NULL), m_aftermathHandle(NULL) {}
...
void NvApi::init()
{
    ...
    if (NULL != m_nvApiDll) {
        nvApiQueryInterface =
            (PFN_NVAPI_QUERYINTERFACE)dlsym(m_nvApiDll, "nvapi_QueryInterface");
        ...
        if (initialized) {
            ...
            nvApiEnumPhysicalGPUs =
                (PFN_NVAPI_ENUMPHYSICALGPUS)nvApiQueryInterface(NVAPI_ENUMPHYSICALGPUS);
            nvApiGpuGetMemoryInfo =
                (PFN_NVAPI_GPUGETMEMORYINFO)nvApiQueryInterface(NVAPI_GPUGETMEMORYINFO);
            nvApiGpuGetFullName =
                (PFN_NVAPI_GPUGETFULLNAME)nvApiQueryInterface(NVAPI_GPUGETFULLNAME);

            nvApiD3D11MultiDrawInstancedIndirect =
                (PFN_NVAPI_MULTIDRAWINDIRECT)nvApiQueryInterface(NVAPI_MULTIDRAWINSTANCEDINDIRECT);
            nvApiD3D11MultiDrawIndexedInstancedIndirect =
                (PFN_NVAPI_MULTIDRAWINDIRECT)nvApiQueryInterface(
                    NVAPI_MULTIDRAWINDEXEDINSTANCEDINDIRECT);

            initialized = true && NULL != nvApiInitialize && NULL != nvApiUnload &&
                          NULL != nvApiEnumPhysicalGPUs && NULL != nvApiGpuGetMemoryInfo &&
                          NULL != nvApiGpuGetFullName && NVAPI_OK == nvApiInitialize();
...
void NvApi::getMemoryInfo(int64_t& _gpuMemoryUsed, int64_t& _gpuMemoryMax)
{
    if (NULL != m_nvGpu) {
        ...
        if (NVAPI_OK == status) {
            ...
            _gpuMemoryUsed = 1024 * int64_t(memInfo.availableDedicatedVideoMemory -
                                            memInfo.curAvailableDedicatedVideoMemory);
            //				BX_TRACE("            dedicatedVideoMemory: %d KiB",
            // memInfo.dedicatedVideoMemory); 				BX_TRACE("   availableDedicatedVideoMemory: %d
            // KiB", memInfo.availableDedicatedVideoMemory); 				BX_TRACE("
            // systemVideoMemory: %d KiB", memInfo.systemVideoMemory); 				BX_TRACE("
            // sharedSystemMemory: %d KiB", memInfo.sharedSystemMemory);
            // BX_TRACE("curAvailableDedicatedVideoMemory: %d KiB",
            // memInfo.curAvailableDedicatedVideoMemory);
        }
    } else {
...
bool NvApi::loadAftermath()
{
    ...
    if (NULL != m_nvAftermathDll) {
        nvAftermathDx11Initialize = (PFN_NVAFTERMATH_DX11_INITIALIZE)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_DX11_Initialize");
        nvAftermathDx11CreateContextHandle = (PFN_NVAFTERMATH_DX11_CREATECONTEXTHANDLE)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_DX11_CreateContextHandle");
        nvAftermathDx12Initialize = (PFN_NVAFTERMATH_DX12_INITIALIZE)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_DX12_Initialize");
        nvAftermathDx12CreateContextHandle = (PFN_NVAFTERMATH_DX12_CREATECONTEXTHANDLE)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_DX12_CreateContextHandle");
        nvAftermathReleaseContextHandle = (PFN_NVAFTERMATH_RELEASECONTEXTHANDLE)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_ReleaseContextHandle");
        nvAftermathSetEventMarker = (PFN_NVAFTERMATH_SETEVENTMARKER)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_SetEventMarker");
        nvAftermathGetData =
            (PFN_NVAFTERMATH_GETDATA)dlsym(m_nvAftermathDll, "GFSDK_Aftermath_GetData");
        nvAftermathGetDeviceStatus = (PFN_NVAFTERMATH_GETDEVICESTATUS)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_GetDeviceStatus");
        nvAftermathGetPageFaultInformation = (PFN_NVAFTERMATH_GETPAGEFAULTINFORMATION)dlsym(
            m_nvAftermathDll, "GFSDK_Aftermath_GetPageFaultInformation");

        bool initialized =
            true && NULL != nvAftermathDx11Initialize &&
            NULL != nvAftermathDx11CreateContextHandle && NULL != nvAftermathDx12Initialize &&
            NULL != nvAftermathDx12CreateContextHandle && NULL != nvAftermathReleaseContextHandle &&
            NULL != nvAftermathSetEventMarker && NULL != nvAftermathGetData &&
            NULL != nvAftermathGetDeviceStatus && NULL != nvAftermathGetPageFaultInformation;
...
bool NvApi::initAftermath(const ID3D11Device* _device, const ID3D11DeviceContext* _deviceCtx)
{
    if (loadAftermath()) {
        ...
        if (1 == result) {
            ...
            BX_WARN(1 == result, "NV Aftermath: nvAftermathDx12CreateContextHandle failed %x",
                    result);
            ...
            switch (result) {
            case int32_t(0xbad0000a):
                BX_TRACE("NV Aftermath: Debug layer not compatible with Aftermath.");
                break;
            default:
                BX_TRACE("NV Aftermath: Failed to initialize.");
                break;
            }
        }

...
void pgTexture::Clear(pgClearFlags clearFlags, const Diligent::float4& color, float depth,
                      uint8_t stencil)
{
    ...
    if ((((int)clearFlags & (int)pgClearFlags::Depth) != 0) ||
        ...
        flags |=
            ((int)clearFlags & (int)pgClearFlags::Stencil) != 0 ? Diligent::CLEAR_STENCIL_FLAG : 0;

        auto dsv = GetDepthStencilView();

        if (dsv) {
            pgApp::s_ctx->ClearDepthStencil(dsv, (Diligent::CLEAR_DEPTH_STENCIL_FLAGS)flags, depth,
                                            stencil,
                                            Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
        }
    }
...
void pgTexture::Bind(uint32_t ID, Shader::ShaderType shaderType,
                     ShaderParameter::Type parameterType)
{
    //
}

void pgTexture::UnBind(uint32_t ID, Shader::ShaderType shaderType,
                       ShaderParameter::Type parameterType)
{
    //
}

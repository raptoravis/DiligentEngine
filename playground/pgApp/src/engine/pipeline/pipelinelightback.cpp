...
PipelineLightBack::PipelineLightBack(std::shared_ptr<pgRenderTarget> rt,
    ...
    : base(rt), m_pGBufferRT(GBufferRT)
{
    CreatePipelineState();
}
...
void PipelineLightBack::CreatePipelineState()
{
    ...
    RefCntAutoPtr<IShader> pVS;
    {
        ...
        // CreateUniformBuffer(pgApp::s_device, sizeof(float4x4), "VS constants CB",
        // &m_VSConstants);
    }
    ...
    // m_pPSO->GetStaticVariableByName(SHADER_TYPE_PIXEL,
    // "ScreenToViewParams")->Set(m_ScreenToViewParamsCB);
    ...
    m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "DiffuseTextureVS")
        ->Set(diffuseTex->getShaderResourceView());

    auto specularTex = GbufferRT->GetTexture(pgRenderTarget::AttachmentPoint::Color2);
    m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "SpecularTextureVS")
        ->Set(specularTex->getShaderResourceView());

    auto normalTex = GbufferRT->GetTexture(pgRenderTarget::AttachmentPoint::Color3);
    m_pSRB->GetVariableByName(SHADER_TYPE_PIXEL, "NormalTextureVS")
        ->Set(normalTex->getShaderResourceView());
